defmodule PersonalSiteWeb.DesktopLiveView do
  ...
  def handle_event("run_program", %{"path" => path}, socket) do
    # {:ok, pid } = PersonalSite.Program.run(path)
    # {:noreply, assign(socket, pids: MapSet.put(socket.assigns.pids, pid))}
    {:ok, program} = PersonalSite.Program.run(path)
    {:noreply, assign(socket, programs: Map.put(socket.assigns.programs, program.pid, program))}
  end

  def handle_event("close_program", %{"pid" => pid}, socket) do
    # {:ok, assign(socket, pids: MapSet.delete(socket.assigns.pids, pid))}
    {:noreply, assign(socket, programs: Map.delete(socket.assigns.programs, pid))}
  end
  ...
  def handle_event("explorer_open_file", %{"pid" => pid, "path" => path}, socket) do
    ...
    {:noreply, assign(socket, programs: Map.merge(socket.assigns.programs, new_programs))}
  end
  ...
  def handle_event("open_obj", value, socket) do
    ...
    new_folders = socket.assigns.processes |> Map.put(value["slug"], new_folder)
    {:noreply, assign(socket, processes: new_folders)}
  end

  def handle_event("close", value, socket) do
    new_folders = socket.assigns.processes |> Map.delete(value["slug"])
    {:noreply, assign(socket, processes: new_folders)}
  end

  def handle_event("card_window_moved", value, socket) do
    # moved_folder = Map.merge(%{x: value["x"], y: value["y"]}, folder_lookup(value["slug"]))
    # new_folders = socket.assigns.processes |> Map.put(value["slug"], moved_folder)
    # {:noreply, assign(socket, processes: new_folders)}

    moved_program =
      Map.get(socket.assigns.programs, value["pid"])
      |> Map.merge(%{x: value["x"], y: value["y"]})

    {:noreply,
     assign(
       socket,
       programs: Map.put(socket.assigns.programs, value["pid"], moved_program)
     )}
  end
    ...
    case slug do
      "fun_projects" ->
        %PersonalSite.Folder{
          ...
          title: "Fun Projects",
          description:
            "Some of my past projects. Collection of partially finished ideas, games and other side projects I worked on -- mostly to learn new things."
        }
      ...
      "work_projects" ->
        %PersonalSite.Folder{
          ...
          title: "Work Projects",
          description:
            "Some descriptions of Actual Projects I've worked on.  Its a little slice of my Resume.  You can also just check out my actual resume"
        }
      ...
      slug ->
        %PersonalSite.Folder{slug: slug, title: "Unknown", description: "???"}
    end
  end
end
